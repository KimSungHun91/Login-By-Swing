...
public class JoinForm extends JDialog implements ActionListener {
	...
	private void setDisplay() {
		...
		TitledBorder tBorder = new TitledBorder(new EmptyBorder(5, 10, 5, 10), "- Input your infomation");
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btnJoin) {
			...
			if (flag) {
				User user = new User(inputs[ID].getText(), inputs[PW].getText(), inputs[NAME].getText(),
						inputs[NICK].getText(), rbtnMale.isSelected() ? "male" : "female");
				owner.addUser(user);
				closeDlg();
			}
			JOptionPane.showMessageDialog(this, msg, "Infomation", JOptionPane.INFORMATION_MESSAGE);
		} else {
			closeDlg();
		}
	}
